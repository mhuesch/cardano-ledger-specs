...
To separate the concerns of moving funds and exercising rights to participate in
the protocol, we discern between different types of addresses:
\begin{description}
\item[Value Addresses] are used to transfer value. Whenever a transaction spends
  a UTxO entry, it has to contain a witness for the value address of that UTxO
  entry.

  In its simplest form, a value address is associated with a payment key pair
  \((skp, vkp)\). The address then consists of a cryptographic hash
  \(\hash{vkp}\) of the verification key \(vkp\). A witness for such an address
  contains the verification key \(vkp\) and a signature of the signing key
  \(skp\).

  We also support value addresses that need signatures from multiple parties.
  Those also contain a hash, not of a verification key, but of a validator
  script. The script can be written either in Plutus, or in a mini-language for
  expressing multi-sig conditions. In the latter case, the script will be
  interpreted natively by Cardano\footnote{Note that the encoding of a script
    that is hashed should include a tag for the language, so that the same
    script text in different languages will result in different hashes.}.

  A witness for a multi-sig value address contains the validator script matching
  the hash in the address, and a set of single key witnesses (i.e., pairs of
  verification keys and signatures of corresponding signing keys). The validator
  script will determine whether those signatures are sufficient for the funds to
  be spent. For details, and an example language, see~\citep{multi-sig-scripts}.

  Note that this form of addresses, which in Shelley will only be used to
  support multi-sig, is also a natural step in the direction of supporting full
  Plutus scripts in Cardano, which is planned for Goguen.

\item[Stake Addresses] are used to delegate stake, and to claim rewards for
  participating in the protocol.

  Just like value addresses, stake addresses can take the two forms of either the
  hash of a verification key, or the hash of a validation script.

\item[Addresses] combine value addresses and stake addresses. Each UTxO entry
  belongs to an address that determines who is eligible to spend the contained
  funds, and who (if anyone) can exercise the stake rights associated with those
  funds.
...
time over all the registered stake addresses\footnote{As explained in
  \cref{stake-key-registration-certificates}, stake addresses need to be
