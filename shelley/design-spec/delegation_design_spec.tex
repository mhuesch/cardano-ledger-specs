...
\item[Value Addresses] are used to transfer value. Whenever a transaction spends
  ...
  \(skp\). We call those addresses \emph{single key addresses} or simply
  \emph{key addresses}, and the corresponding witnesses \emph{single key
    witnesses} or \emph{key witnesses}.
  ...
  interpreted natively by Cardano\footnote{Note that the encoding of a script
    ...
    script text in different languages will result in different hashes.}. We
  will call those addresses \emph{multi-sig addresses}.
  ...
  witnesses. The validator script will determine whether those signatures
  are sufficient for the funds to be spent. For details, and an example
  language, see~\citep{multi-sig-scripts}.

  Note that multi-sig addresses are also a natural step in the direction of
  supporting full Plutus scripts in Cardano, which is planned for Goguen.

\item[Stake Addresses] are used to delegate stake, and to claim rewards for
  participating in the protocol.

  Just like value addresses, stake addresses can be either single key addresses
  or multi-sig addresses.
