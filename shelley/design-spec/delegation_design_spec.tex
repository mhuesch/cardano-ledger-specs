...
and the rights (and obligations) in the Proof of Stake protocol that are
associated with those funds. We achieve this separation by introducing
another type of address: while the right to move funds are tied to a
\emph{value address}, the right (and obligation) to take part in the PoS
is tied to a \emph{stake address}. The association between the two is
that a value address can contain a reference to a stake address.

Except for special classes of value addresses with no stake
rights\footnote{Enterprise,
  ...
  addresses are special addresses used in the initial distribution of Ada.},
all value addresses have stake rights corresponding to the funds
at the address. To exercise these stake rights, and to receive staking
rewards, each value address must be associated with a \emph{registered}
stake address. This involves registering a stake address on the chain
and using value addresses that refer to this stake address. Each
registered stake address has a corresponding \emph{reward account} which
is used to hold accumulated staking rewards. To exercise stake rights
associated with a registered stake address, those rights must be
\emph{delegated} to a registered stake pool.
...
key. Rewards can accumulate in the reward account of the stake addresses
over multiple epochs and can be reclaimed as part of a transaction using
the stake address as an input.

Thus, the overall structure is that value addresses with stake rights
are associated with a stake address, and the stake address delegates to
a stake pool, and the stake pool takes part in the PoS protocol. Private
staking works in exactly the same way, using a stake address, and
delegating, but using a private stake pool.

To control the conditions of use of an address, value addresses and
stake addresses can be based on a cryptographic key pair. In this case
the holder of the single private key controls the use of the address,
as their signature is required to spend from the value address, or
delegate or withdraw rewards from a stake address.

Value addresses and stake addresses can also be based on scripts, where
the script provides the conditions in which the address can be used. In
the Shelley release the only\footnote{The Goguen release will extend this
with Plutus Core, allowing scripts written in Plutus, Marlowe, or other
languages that compile to Plutus Core.} available script language is one
that is specialised to express multi-signature conditions: such that N
out of M cryptographic keys are required to sign to use the address.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[ align = center
    ...
    \node (valaddr)    [entity, left = of owner]    {Value address};
    \node (registeredStakeAddr)    [entity, below = of owner]   {Registered stake address};
    \node (pkey)    [entity, left = of registeredStakeAddr]     {Payment key or script hash};
    \node (racc)    [entity, right = of registeredStakeAddr]    {Reward account};
    \node (rewards) [concept, below = of racc]   {Rewards};
    \node (funds)   [concept, below = of pkey]   {Funds};
    \node (srights) [concept, below = of registeredStakeAddr]   {Staking rights};
    ...
    %%
    \draw[->] (owner) edge node[above] {owns} (valaddr);
    \draw[->] (valaddr) edge node[left] {contains} (pkey);
    \draw[->] (valaddr) edge node [above, rotate=-40]{refers to} (registeredStakeAddr);
    \draw[->] (owner) edge node[left] {owns} (registeredStakeAddr);
    \draw[->] (owner) edge node[left] {owns} (racc);

    \draw[<->] (registeredStakeAddr) edge node[anchor=south] {is associated\\ with a} (racc);
    \draw[->] (racc) edge node[above, rotate=90] {collects} (rewards);
    \draw[->] (pkey) edge node[anchor=west] {has} (funds);
    \draw[->] (registeredStakeAddr) edge node[anchor=west] {has} (srights);
...
\item Each normal pubkey address\footnote{As opposed to script
  addresses}\todo{update to the new address nomenclature}
...
\item Many value addresses can refer to the same stake address. For instance,
  different outputs of a transaction can specify the that the stake associated
  with those output should be transferred to the same stake key.
\item Each stake address has a unique reward account associated with it, and a
  reward account is only associated with one stake key.
...
\item A stake address is associated with exactly zero or one stake pools at any
  ...
  be multiple stake addresses delegating or pledging to the pool).
\end{itemize}
...
\begin{figure*}[ht]
  ...
  %% Nodes
  %%
  \node (valueAddr) [] {Value address};
  \node (singleKeyAddr) [above = of valueAddr] {Single key address};
  \node (scriptAddr) [right = of singleKeyAddr] {Script address};
  \node (utxo) [left = of valueAddr] {UTxO};
  \node (pkey) [above = of singleKeyAddr] {Payment key};
  \node (registeredStakeAddr) [below = of valueAddr] {Registerd stake address};
  \node (spool) [left = of registeredStakeAddr] {Stake pool};
  \node (racc) [right = of registeredStakeAddr] {Reward account};
  \node (script) [above = of scriptAddr] {Script};
  ...
  \draw[-] (utxo) edge node[pos=0.1]  {$n$}
                       node[pos=0.85] {$1$}
           (valueAddr);
  \draw[{Triangle[open]}-] (valueAddr) edge
                           (singleKeyAddr);
  \draw[{Triangle[open]}-] (valueAddr) edge
                           (scriptAddr);
  \draw[-] (registeredStakeAddr) edge node[pos=0.1]  {$1$}
  ...
  \draw[-] (valueAddr) edge node[pos=0.1] {$n$}
                            node[pos=0.9] {$0 \ldots 1$}
           (registeredStakeAddr);
  \draw[-] (singleKeyAddr) edge node[pos=0.1]  {$n$}
                       node[pos=0.85] {$1$}
           (pkey);
  \draw[-] (scriptAddr) edge node[pos=0.1]  {$1$}
                       node[pos=0.85] {$1$}
           (script);
  \end{tikzpicture}
...
In Shelley, an address has to provide information on two things: who has the
right to spend funds in that address, and who has the right to delegate the
stake associated with funds in that address to a stake pool. To separate those
two concerns, we distinguish between \emph{value addresses} and \emph{stake
  addresses}. Value addresses and stake addresses are building blocks for
(overall) addresses: an address contains a value address, and may also contain a
stake address.

\todo[inline]{Suggest restructuring this subsection based on the following subsubsections. So instead of the description list for key addresses, and stake addresses, turn those into their own subsubsections.

Also clearly document the structure of value and script addresses. Suggest that if we're uising $\beta$ for the staking reference we use $\alpha$ for the payment credential!}

\subsubsection{Key addresses}

\subsubsection{Script addresses}

\subsubsection{Value addresses}

\subsubsection{Stake addresses}

\subsubsection{Terminology of addresses}

\begin{description}
\item[value address] is either a bootstrap address or a new Shelley address which consists of a payment credential and a staking reference.
\item[stake address] consists of a staking credential
\item[payment credential] is either the hash of a verification key or the hash of a validation script
\item[staking credential] is either the hash of a verification key or the hash of a validation script
\item[staking reference] is a reference to a stake address is either a base form, a pointer form or a null form
\item[key address] is a value or stake address where the contained credential uses a single key pair
\item[script address] is a value or stake address where the contained credential uses a validation script
\item[base address] is a value address where the staking reference use the base form
\item[pointer address] is a value address where the staking reference use the pointer form
\item[enterprise address] is a value address where the staking reference use the null form
\item[bootstrap address] is a value address as introduced in Byron.
\item[AVVM address] are special addresses used in the initial distribution of Ada.
\end{description}
...
Bootstrap addresses were introduced in Byron. In Byron they were interpreted as
having stake rights but those stake rights were always delegated to a fixed set
of single key stake addresses specified in the genesis block, controlled by the
...
Finding the stake addresses of a wallet (and in particular the corresponding
reward addresses) can be done either by just reading off all the stake addresses
of addresses found via their value address, or by performing a second search,
...
certificates, and cache the metadata, with the pool's public stake pool key hash
\(\mathcal{H}(sk_\text{pool})\) as key.
...
\item[\cref{multi-sig-delegation} Multi-Sig Delegation] \todo{describe how we
  add multi-sig delegation.}
